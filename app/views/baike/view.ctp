<div class="contentMain">
		<div class="article">
			<div class="titleContent">
				<div class="title"><?php echo $this->data['Article']['title'];?></div>
				<div class="shareButton">
					...
					<span class="sina_weibo">
						...
						(function(){
						  ...
						  var param = {
							...
							title:'<?php echo $this->data['Article']['title'];?>', /**分享的文字内容(可选，默认为所在页面的title)*/
							pic:'<?php echo $this->data['ArticleWeibo']['img'];?>', /**分享图片的路径(可选)*/
					...
					<span class="tencent_weibo">
						...
						<script type="text/javascript">
							function postToWb(){
								...
								var _pic = encodeURI('<?php echo $this->data['ArticleWeibo']['img'];?>');//（例如：var _pic='图片url1|图片url2|图片url3....）
					...
					<script type="text/javascript">
						...
						url:location.href,
						desc:'',/*默认分享理由(可选)*/
						summary:'<?php echo $this->data['ArticleWeibo']['content'];?>',/*摘要(可选)*/
						title:'<?php echo $this->data['Article']['title'];?>',/*分享标题(可选)*/
						site:'豆壳网',/*分享来源 如：腾讯网(可选)*/
						pics:'<?php echo $this->data['ArticleWeibo']['img'];?>' /*分享图片的路径(可选)*/
						};
						var s = [];
						for(var i in p){
				...
				<div class="published">
					<span class="bold">来源：</span><?php echo $this->data['Article']['from'];?> 
					<!-- <span class="bold"> 点击：</span>2088 -->
				</div>
			</div>
			<div class="content">
				<div class="baike">
				  	<?php echo $this->data['ArticleContent'][0]['content']?>
				</div>
			</div>
			<!-- 
			...
			<div class="categorys">
				...
				<?php foreach($this->data['Category'] as $category):?>
					<?php echo $this->Html->link($category['title'], '/Baike/search/category:' . $category['title']);?>					
				<?php endforeach;?>
			</div>			
			<div class="tags">
				<strong>标签：</strong>
				<?php foreach($this->data['Tag'] as $tag):?>
					<?php echo $this->Html->link($tag['title'], '/Baike/search/tag:' . $tag['title']);?>					
				<?php endforeach;?>
			</div>
			<!-- 相关视频 -->
		...
		<div class="weibo">
		  <div class="expert">
				<div class="img">
					<?php echo $this->Html->link($this->Html->image($this->data['User']['myface'], array('width' => 65, 'alt' => $this->data['User']['username'])), '/weibo/toWeibo/'.$this->data['ArticleWeibo']['uid'].'/'.$this->data['ArticleWeibo']['sid'], array('escape' => false, 'target' => '_blank'));?>					
				</div>
				<div class="published">
					<span class="time">发布于  <?php echo date('m月d日 H:i', strtotime($this->data['ArticleWeibo']['created']));?></span>				
					<div class="red bold">
						<?php echo $this->Html->link($this->data['User']['username'], '/weibo/toWeibo/'.$this->data['ArticleWeibo']['uid'].'/'.$this->data['ArticleWeibo']['sid'], array('class' => 'red bold', 'target' => '_blank'));?>
					</div>
				</div>
				<div class="content">
					<?php echo $this->data['ArticleWeibo']['content'];?>
					<?php echo $this->Html->link('[马上关注]', '/weibo/toWeibo/'.$this->data['ArticleWeibo']['uid'].'/'.$this->data['ArticleWeibo']['sid'], array('class' => 'red', 'target' => '_blank'));?> 
				</div>
				<div class="div_c"></div>
		  </div>
			<div class="commentInput">
				<div class="top">&nbsp;</div>
				<div class="formBody">
					<?php echo $this->Html->div('submitButton', '&nbsp;', array('mid' => $this->data['Article']['id'], 'sType' => 'baike'));?>
					...
					<div class="options">
						...
						<?php echo $this->Form->hidden('ArticleWeibo.sid');?>
						<?php echo $this->Form->hidden('Article.id');?>
						<?php echo $this->Form->hidden('ArticleWeibo.uid');?>
					</div>					
				</div>
		  ...
		  <div class="replyList">
				...
				<div class="reply">
					...
					<div class="contentBody">
						...
						<div class="published">
							<span class="time">发布于 <?php echo date('m月d日 H:i', $item['ArticleComment']['created']);?></span>				
							<div class="red bold"><?php echo $item['User']['username']?></div>
						</div>
						<div class="content"><?php echo $item['ArticleComment']['content']?></div>
					</div>
					<div class="div_c"></div>
				</div>			
				<?php endforeach;?>	
				<!-- 回评列表结束 -->
				<div class="pageShow alignRight">

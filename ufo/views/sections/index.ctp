  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td class="menu-function"><label>
        <input type="button" name="Submit" value="新增标签" onClick="self.location.href='<?php echo $this->Html->url('/sections/edit');?>'"/>
        </label>
      </td>
  ...
  <form id="infolist" name="infolist" method="post" action="#" style="margin:0px;">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
      <tr>
        ...
        <th><?=$paginator->sort('分类名称', 'Section.title')?></td>
      ...
      <tr>
        ...
        <td><?php echo $item['Section']['id'];?></td>
        <td><?php echo $this->Html->link($item['Section']['title'], '/sections/edit/'.$item['Section']['id']);?></td>		
        <td>
        	<?php echo $this->Html->link('编辑', '/sections/edit/'.$item['Section']['id'], array('target' => '_self'))?>
			<?php echo $this->Html->link('删除', '/sections/delete/'.$item['Section']['id'], array('target' => '_self'), '确认是否删除该版块')?>
		</td>
      </tr>
	  <?php
	  	endforeach;
